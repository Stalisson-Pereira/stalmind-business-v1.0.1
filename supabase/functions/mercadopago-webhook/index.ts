...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!payment) {
      console.warn(
        "[mercadopago-webhook] Payment local não encontrado. Tentando reconstruir pelo external_reference:",
        { providerId, externalReference },
      );

      // O PIX já foi criado no Mercado Pago. Se o insert local falhou,
      // reconstruímos o ledger aqui usando a referência assinada pelo
      // próprio pagamento. Isso permite que o webhook continue sendo a
      // fonte de verdade mesmo quando o primeiro insert sofreu erro.
      const referenceParts = externalReference.split("_");
      const workspaceIdFromReference = referenceParts[1] || null;
      const planFromReference = getPlanFromExternalReference(externalReference);

      if (workspaceIdFromReference && planFromReference) {
        const amount = Number(mpPayment.transaction_amount || 0);
        const notes = JSON.stringify({
          provider: "mercado_pago",
          provider_payment_id: providerId,
          external_reference: externalReference,
          plan: planFromReference,
          currency: "BRL",
          payer_email: mpPayment.payer?.email || null,
          mp_status: mpPayment.status || null,
          mp_status_detail: mpPayment.status_detail || null,
          reconstructed_by_webhook: true,
        });

        const { data: reconstructed, error: reconstructError } = await supabase
          .from("payments")
          .insert({
            workspace_id: workspaceIdFromReference,
            amount,
            payment_method: "pix",
            status: getLocalStatus(mpPayment.status),
            payment_date: new Date().toISOString(),
            reference: providerId,
            notes,
          })
          .select("id, workspace_id, amount, status, notes, payment_method, reference")
          .single();

        if (!reconstructError && reconstructed) {
          payment = reconstructed;
          console.log("[mercadopago-webhook] Payment reconstruído:", reconstructed.id);
        } else {
          console.error("[mercadopago-webhook] Falha ao reconstruir payment:", reconstructError);
        }
      }
      ...
      if (!payment) {
        return json({
          received: true,
          payment_found: false,
          mercadopago_payment_id: providerId,
